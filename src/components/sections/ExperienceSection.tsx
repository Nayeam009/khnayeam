...
interface ExperienceContent {
  main: { name: string; role: string; duration: string; description: string; tags: string[]; url: string };
  projects: { name: string; url: string; icon: string; role: string; desc: string; tags: string[] }[];
  skill_categories: { category: string; skills: string[] }[];
}
...
const ExperienceSection = () => {
  const { data } = useSiteContent<ExperienceContent>("experience");
        ...
        <MotionCard index={0} className="mt-10">
          <div className="bento-card bg-gradient-to-br from-primary/5 via-card to-card group">
            ...
            <div className="flex flex-col md:flex-row md:items-start gap-6">
              <motion.div whileHover={{ rotate: 5, scale: 1.05 }} className="circle-icon circle-icon-lg bg-primary/15 flex-shrink-0">
                <Briefcase className="text-primary" size={28} />
              </motion.div>
              <div className="flex-1">
                <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3 mb-2">
                  <h3 className="font-bold text-xl md:text-2xl text-foreground font-serif">{content.main.name}</h3>
                  <span className="pill-tag pill-tag-primary w-fit">{content.main.role}</span>
                  <span className="pill-tag pill-tag-accent w-fit">{content.main.duration}</span>
                </div>
                <p className="text-sm text-muted-foreground leading-relaxed mt-2 max-w-2xl">{content.main.description}</p>
                <div className="flex flex-wrap gap-2 mt-4">
                  {content.main.tags.map(t => <span key={t} className="pill-tag pill-tag-muted text-[10px]">{t}</span>)}
                </div>
              </div>
              <a href={content.main.url} target="_blank" rel="noopener noreferrer" className="flex-shrink-0 self-start w-full sm:w-auto">
          ...
          {content.projects.map((p, i) => {
            ...
            return (
              <MotionCard key={p.name} index={i + 1} className="group h-full">
                <a href={p.url} target="_blank" rel="noopener noreferrer" className="bento-card flex flex-col h-full">
                  <div className="flex items-center justify-between mb-4">
                    <motion.div whileHover={{ rotate: -5 }} className="circle-icon circle-icon-md bg-primary/10 group-hover:bg-primary transition-all duration-300">
                  ...
                  <div className="flex flex-wrap gap-1.5 mt-4">
                    {p.tags.map(t => <span key={t} className="pill-tag pill-tag-muted text-[10px]">{t}</span>)}
                  </div>
                </a>
              </MotionCard>
            );
            ...
            <div className="flex flex-col lg:flex-row">
              <div className="flex-1 p-6 sm:p-8 lg:p-10 flex flex-col justify-center">
                <div className="flex items-center gap-3 mb-4">
                  <div className="circle-icon circle-icon-md bg-primary/10"><Zap className="text-primary" size={20} /></div>
                  <span className="pill-tag pill-tag-primary text-xs">Core Expertise</span>
                </div>
                <h3 className="font-bold text-foreground text-2xl sm:text-3xl font-serif mb-2">Technical Skills & Competencies</h3>
                <p className="text-sm text-muted-foreground mb-6 max-w-lg">Build high-performance solutions across web development, agriculture, and leadership domains.</p>
                <div className="space-y-5">
                  {content.skill_categories.map((group) => (
                    <div key={group.category} className="space-y-2.5">
                      <h4 className="text-sm font-bold text-foreground tracking-wide">{group.category}</h4>
                      <div className="flex flex-wrap gap-2">
                        {group.skills.map((skill) => (
                          <motion.span key={skill} whileHover={{ scale: 1.05 }} className="pill-tag pill-tag-muted cursor-default">{skill}</motion.span>
                        ))}
                      </div>
                    </div>
                  ))}
              ...
              <div className="lg:w-[45%] p-4 sm:p-6">
                <ImageAccordion items={skillImages} defaultActive={2} className="h-[280px] sm:h-[350px] lg:h-full lg:min-h-[420px]" />
              </div>
            </div>
          </div>
        </MotionCard>
